...
function AuditTimelineAnalysis({ task, onClose }: { task: Task; onClose: () => void }) {
  ...
  return <section className="panel analysis"><div className="section-title"><h2>{t('audit.title')}</h2><button onClick={onClose}>{t('actions.close')}</button></div><p><strong>{task.name}</strong> · {formatBytes(task.inputSize)} · <code>{task.sha256}</code></p>{error && <p role="alert">{error}</p>}{timeline && <><div className="metrics"><Metric metricKey="audit.totalLines" value={String(timeline.summary.totalLines)} /><Metric metricKey="audit.validEvents" value={String(timeline.summary.validEvents)} /><Metric metricKey="audit.writeEvents" value={String(timeline.summary.writeEvents)} /><Metric metricKey="audit.unknownLines" value={String(timeline.summary.unknownLines)} /><Metric metricKey="audit.parseErrors" value={String(timeline.summary.parseErrors)} /></div><p className="notice">{t('audit.safetyBoundary')} {t('audit.payloadCaveat')}</p><div className="filters"><label>{t('log.from')}<input type="datetime-local" onChange={(event) => update('from', event.target.value ? new Date(event.target.value).toISOString() : '')} /></label><label>{t('log.to')}<input type="datetime-local" onChange={(event) => update('to', event.target.value ? new Date(event.target.value).toISOString() : '')} /></label><label>{t('audit.verb')}<select value={filters.verb} onChange={(event) => update('verb', event.target.value)}><option value="">—</option>{['create','update','patch','delete','deletecollection'].map((verb) => <option key={verb}>{verb}</option>)}</select></label><label>{t('audit.username')}<input value={filters.username} onChange={(event) => update('username', event.target.value)} /></label><label>{t('audit.resource')}<input value={filters.resource} onChange={(event) => update('resource', event.target.value)} /></label><label>{t('audit.namespace')}<input value={filters.namespace} onChange={(event) => update('namespace', event.target.value)} /></label></div><div className="table-wrap"><table><thead><tr><th>{t('log.time')}</th><th>{t('audit.verb')}</th><th>{t('audit.username')}</th><th>{t('audit.userAgent')}</th><th>{t('audit.sourceNetwork')}</th><th>{t('audit.resource')}</th><th>{t('audit.namespace')}</th><th>{t('audit.object')}</th><th>{t('audit.responseCode')}</th></tr></thead><tbody>{timeline.items.map((event: AuditEvent) => <tr key={event.eventId}><td>{logTime(event.observedAt, locale, t)}</td><td>{event.verb}</td><td>{event.username}</td><td>{event.userAgent}</td><td>{event.sourceNetwork}</td><td>{event.apiGroup || t('value.core')}/{event.resource}</td><td>{event.namespace || t('value.clusterScoped')}</td><td>{event.displayName || '—'}</td><td>{event.responseCode || '—'}</td></tr>)}{timeline.items.length === 0 && <tr><td colSpan={9} className="empty">{t('audit.empty')}</td></tr>}</tbody></table></div><div className="pager"><button disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>{t('audit.previous')}</button><span>{t('pagination.records', { page, count: timeline.total })}</span><button disabled={page * timeline.pageSize >= timeline.total} onClick={() => setPage((value) => value + 1)}>{t('audit.next')}</button></div></>}</section>;
}
